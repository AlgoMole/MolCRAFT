...
class ScorePosNet3D(nn.Module):
    ...
    def forward(self, protein_pos, protein_v, batch_protein, init_ligand_pos, init_ligand_v, batch_ligand,
        ...
        # time embedding
        if self.time_emb_dim > 0:
            raise ValueError('time embedding is not supported')
            if self.time_emb_mode == 'simple':
    ...
    def get_diffusion_loss(
        ...
        if time_step is None:
            ...
            # raise NotImplementedError('time_step is not None')
            pt = torch.ones_like(time_step).float() / self.num_timesteps
            time_step_v = time_step
        ...
        log_v_model_prob = self.q_v_posterior(log_ligand_v_recon, log_ligand_vt, time_step_v, batch_ligand)
        log_v_true_prob = self.q_v_posterior(log_ligand_v0, log_ligand_vt, time_step_v, batch_ligand)
    ...
    @torch.no_grad()
    def likelihood_estimation_decouple(
            self, protein_pos, protein_v, batch_protein, ligand_pos, ligand_v, batch_ligand, time_step_cont, time_step_disc
    ):
        protein_pos, ligand_pos, _ = center_pos(
            protein_pos, ligand_pos, batch_protein, batch_ligand, mode='protein')
        
        assert (time_step_cont == self.num_timesteps).all() or (time_step_cont < self.num_timesteps).all()
        assert (time_step_disc == self.num_timesteps).all() or (time_step_disc < self.num_timesteps).all()
        
        if (time_step_cont == self.num_timesteps).all() and (time_step_disc == self.num_timesteps).all():
            kl_pos_prior = self.kl_pos_prior(ligand_pos, batch_ligand)
            log_ligand_v0 = index_to_log_onehot(ligand_v, self.num_classes)
            kl_v_prior = self.kl_v_prior(log_ligand_v0, batch_ligand)
            return kl_pos_prior, kl_v_prior

        # Perturb continuous variable (ligand_pos)
        a_cont = self.alphas_cumprod.index_select(0, time_step_cont)  # (num_graphs, )
        a_pos = a_cont[batch_ligand].unsqueeze(-1)  # (num_ligand_atoms, 1)
        pos_noise = torch.zeros_like(ligand_pos)
        pos_noise.normal_()
        ligand_pos_perturbed = a_pos.sqrt() * ligand_pos + (1.0 - a_pos).sqrt() * pos_noise

        # Perturb discrete variable (ligand_v)
        log_ligand_v0 = index_to_log_onehot(ligand_v, self.num_classes)
        ligand_v_perturbed, log_ligand_vt = self.q_v_sample(log_ligand_v0, time_step_disc, batch_ligand)
        ...
        preds = self(
            ...
            init_ligand_v=ligand_v_perturbed,
            batch_ligand=batch_ligand,
            time_step=time_step_cont  # Use continuous time step for position
        )

        pred_ligand_pos, pred_ligand_v = preds['pred_ligand_pos'], preds['pred_ligand_v']
        if self.model_mean_type == 'C0':
            pos_model_mean = self.q_pos_posterior(
                x0=pred_ligand_pos, xt=ligand_pos_perturbed, t=time_step_cont, batch=batch_ligand)
        else:
            raise ValueError

        # Atom type (discrete variable)
        log_ligand_v_recon = F.log_softmax(pred_ligand_v, dim=-1)
        log_v_model_prob = self.q_v_posterior(log_ligand_v_recon, log_ligand_vt, time_step_disc, batch_ligand)
        log_v_true_prob = self.q_v_posterior(log_ligand_v0, log_ligand_vt, time_step_disc, batch_ligand)

        # Compute KL divergences
        kl_pos = self.compute_pos_Lt(pos_model_mean=pos_model_mean, x0=ligand_pos,
                                    xt=ligand_pos_perturbed, t=time_step_cont, batch=batch_ligand)
        kl_v = self.compute_v_Lt(log_v_model_prob=log_v_model_prob, log_v0=log_ligand_v0,
                                log_v_true_prob=log_v_true_prob, t=time_step_disc, batch=batch_ligand)
        return kl_pos, kl_v
    ...
    def sample_diffusion(self, protein_pos, protein_v, batch_protein,
                         ...
                         num_steps=None, center_pos_mode=None, pos_only=False, time_scheduler=None):
        ...
        print(f'num_steps: {num_steps}, time_seq: {time_seq[::100]}')
        if time_scheduler is not None:
            print(f't_cont: {time_scheduler[::100, 1].cpu().numpy()}')
            print(f't_disc: {time_scheduler[::100, 0].cpu().numpy()}')

            print(f'First few t_cont steps: {time_scheduler[time_seq[:10], 1].cpu().numpy()}')
            print(f'First few t_disc steps: {time_scheduler[time_seq[:10], 0].cpu().numpy()}')
        for i in tqdm(time_seq, desc='sampling', total=len(time_seq)):
            ...
            if time_scheduler is not None:
                time_disc, time_cont = time_scheduler[i]
                assert time_disc < num_steps and time_cont < num_steps
                # round and clip to [0, num_steps)
                # time_cont = torch.clamp(torch.round(time_cont), 0, num_steps - 1)
                # time_disc = torch.clamp(torch.round(time_disc), 0, num_steps - 1)
                time_cont = torch.full(size=(num_graphs,), fill_value=time_cont, dtype=torch.long, device=protein_pos.device)
                time_disc = torch.full(size=(num_graphs,), fill_value=time_disc, dtype=torch.long, device=protein_pos.device)
            else:
                time_cont, time_disc = t, t
            ...
            if self.model_mean_type == 'noise':
                ...
                pos0_from_e = self._predict_x0_from_eps(xt=ligand_pos, eps=pred_pos_noise, t=time_cont, batch=batch_ligand)
            ...
            pos_model_mean = self.q_pos_posterior(x0=pos0_from_e, xt=ligand_pos, t=time_cont, batch=batch_ligand)
            pos_log_variance = extract(self.posterior_logvar, time_cont, batch_ligand)
            # no noise when t == 0
            ...
            if not pos_only:
                ...
                log_model_prob = self.q_v_posterior(log_ligand_v_recon, log_ligand_v, time_disc, batch_ligand)
