...
def sample_diffusion_ligand_surface(model, data, num_samples, batch_size=16, device='cuda:0',
                            num_steps=None, pos_only=False, center_pos_mode='protein',
                            sample_num_atoms='prior', time_scheduler=None):
...
if __name__ == '__main__':
    ...
    parser.add_argument('config', type=str, default='configs/sampling_rect_scheduler.yml')
    ...
    time_scheduler_path = config.sample.time_scheduler_path
    # if time_scheduler_path is not None:
    #     time_scheduler = torch.load(time_scheduler_path, map_location=args.device)
    #     sample_steps = config.sample.num_steps
    #     t_steps = time_scheduler / time_scheduler.max() * (sample_steps - 1)
    #     if t_steps.shape != (sample_steps, 2):
    #         # interpolate t_steps
    #         # Generate the desired new indices
    #         new_indices = np.linspace(0, len(t_steps), num=sample_steps)

    #         # Interpolate t_steps
    #         t_steps_interpolated = np.zeros((sample_steps, 2))  # Assuming 2 columns in t_steps
    ...
    #         t_steps = torch.from_numpy(t_steps_interpolated).to(device=args.device, dtype=torch.float32)
    #     assert t_steps.shape == (sample_steps, 2), f"t_steps: {t_steps.shape}"
    #     time_scheduler = t_steps.int()
    #     logger.info(f"Time scheduler loaded from {time_scheduler_path}")
    time_scheduler_path = config.sample.time_scheduler_path
    if time_scheduler_path is not None:
        # Load the original time scheduler
        time_scheduler = torch.load(time_scheduler_path, map_location=args.device)
        sample_steps = config.sample.num_steps
        N = time_scheduler.shape[0]

        # Scale from [0, N-1] to [0, sample_steps-1]
        time_scheduler = time_scheduler * (sample_steps-1) / time_scheduler.max()
        
        if N != sample_steps:
            # Original tensor has shape [N, 2], need to resize to [sample_steps, 2]
            
            # Generate the original and new indices
            orig_indices = np.arange(N)
            new_indices = np.linspace(0, N-1, num=sample_steps)
            
            # Interpolate time_scheduler
            time_scheduler_interpolated = np.zeros((sample_steps, 2))
            for i in range(2):  # Interpolate each column independently
                time_scheduler_interpolated[:, i] = np.interp(new_indices, orig_indices, time_scheduler[:, i])
            
            time_scheduler = torch.from_numpy(time_scheduler_interpolated).to(device=args.device, dtype=torch.float32)
        
        assert time_scheduler.shape == (sample_steps, 2), f"time_scheduler shape: {time_scheduler.shape}"
        time_scheduler = time_scheduler.int()
    else:
        time_scheduler = None
    pred_pos, pred_v, pred_pos_traj, pred_v_traj, pred_v0_traj, pred_vt_traj, time_list = sample_diffusion_ligand_surface(
