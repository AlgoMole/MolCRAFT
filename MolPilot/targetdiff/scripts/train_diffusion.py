...
if __name__ == '__main__':
    ...
    wandb.init(project='qiukeyue_benchmark_sbdd', config=config, name='targetdiff_crossdock')
    logger.info(args)
    logger.info(config)
        ...
        if it % args.train_report_iter == 0:
            ...
            for k, v in results.items():
                if torch.is_tensor(v) and v.squeeze().ndim == 0:
                    wandb.log(step=it, data={f'train/{k}': v})
                    # writer.add_scalar(f'train/{k}', v, it)
            # writer.add_scalar('train/lr', optimizer.param_groups[0]['lr'], it)
            # writer.add_scalar('train/grad', orig_grad_norm, it)
            # writer.flush()
            wandb.log(step=it, data={'train/lr': optimizer.param_groups[0]['lr'], 
    ...
    def validate(it):
        ...
        # writer.flush()
        wandb.log(step=it, data={'val/loss': avg_loss, 'val/loss_pos': avg_loss_pos, 
