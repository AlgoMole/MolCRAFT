...
def compute_scaffold_diversity(results):
    pocket_scaffold_data = []
    unique_scaffolds = set()
    all_scaffolds = []
    
    for pocket_idx, res in enumerate(tqdm(results, desc='pocket')):
        pocket_results = [r for r in res if r['mol'] is not None]
        mols = [r['mol'] for r in pocket_results]
        scaffolds = [get_scaffold(Chem.MolToSmiles(mol)) for mol in mols if mol is not None]
        scaffolds = [scaf for scaf in scaffolds if scaf is not None]
        
        # Pocket-specific unique scaffolds and ratio
        unique_pocket_scaffolds = set(scaffolds)
        unique_scaffolds.update(unique_pocket_scaffolds)
        all_scaffolds.extend(scaffolds)
        
        unique_ratio = len(unique_pocket_scaffolds) / len(scaffolds) if scaffolds else 0
        pocket_scaffold_data.append({
            "unique_scaffolds": unique_pocket_scaffolds,
            "unique_ratio": unique_ratio,
            "scaffolds": scaffolds
        })

    # Overall unique scaffold ratio
    overall_unique_ratio = len(unique_scaffolds) / len(all_scaffolds) if all_scaffolds else 0

    # Unique ratio averaged over pockets
    pocket_unique_ratios = [pocket['unique_ratio'] for pocket in pocket_scaffold_data]

    
    scaffold_counts = Counter(all_scaffolds)
    total_scaffolds = sum(scaffold_counts.values())
    scaffold_probs = [count / total_scaffolds for count in scaffold_counts.values()]
    scaffold_entropy = -sum(p * np.log(p) for p in scaffold_probs if p > 0)

    print(f"[Scaffold Diversity] Overall Unique Ratio: {overall_unique_ratio:.4f} | Entropy: {scaffold_entropy:.4f} | Avg Unique Ratio: {np.mean(pocket_unique_ratios):.4f}")
    
    return pocket_scaffold_data, overall_unique_ratio, scaffold_entropy
