...
class BFNBase(nn.Module):
    ...
    def continuous_var_bayesian_update(self, t, sigma1, x):
        ...
        # print('t', t.requires_grad, 'sigma1', sigma1.requires_grad, 'x', x.requires_grad, 'mu', mu.requires_grad, 'gamma', gamma.requires_grad)
        return mu, gamma
    ...
    def dtime4continuous_loss(self, i, N, sigma1, x_pred, x, segment_ids=None, batch_mean=True):
    ...
    def dtime4discrete_loss_prob(
        self, i, N, beta1, one_hot_x, p_0, K, n_samples=200, segment_ids=None, batch_mean=True
    ):
        ...
        loss = N * (sender_dist.log_prob(y) - receiver_dist.log_prob(y)).mean(0)  # over n_samples
