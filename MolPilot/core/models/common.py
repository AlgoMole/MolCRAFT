...
#     def __init__(self, in_dim, out_dim, hidden_dim, num_layer=2, bias=True, norm=True, act_fn='relu', act_last=False):
#         super().__init__()
#         DenseMLP = partial(Dense, bias=bias)
#         activation = NONLINEARITIES[act_fn]

#         layers = []
#         for layer_idx in range(num_layer):
#             if layer_idx == 0:
#                 layers.append(DenseMLP(in_dim, hidden_dim, norm=norm, activation=activation))
#             elif layer_idx == num_layer - 1:
#                 layers.append(DenseMLP(hidden_dim, out_dim, activation=activation if act_last else None))
#             else:
#                 layers.append(DenseMLP(hidden_dim, hidden_dim, norm=norm, activation=activation))
    ...
    def __init__(self, in_dim, out_dim, hidden_dim, num_layer=2, norm=True, act_fn='relu', act_last=False):
        ...
        for layer_idx in range(num_layer):
            if layer_idx == 0:
                ...
                nn.init.kaiming_uniform_(layers[-1].weight, nonlinearity=act_fn)
            elif layer_idx == num_layer - 1:
            ...
            if layer_idx < num_layer - 1 or act_last:
                if norm:
                    norm_dim = hidden_dim if layer_idx < num_layer - 1 else out_dim
