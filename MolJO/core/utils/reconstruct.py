...
def convert_ob_mol_to_rd_mol(ob_mol, struct=None, sanitize=True):
...
def reconstruct_from_generated(xyz, atomic_nums, aromatic=None, basic_mode=True, sanitize=True):
    """
    ...
    mol.PerceiveBondOrders()
    rd_mol = convert_ob_mol_to_rd_mol(mol, struct=xyz, sanitize=sanitize)
    if sanitize:
        try:
            # Post-processing
